...
function HeroScene() {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 1, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", pointerEvents: "none" }}>
      {/* Multi-layer CD shimmer glow */}
      <div style={{
        position: "absolute",
        width: "80vmin", height: "80vmin",
        borderRadius: "50%",
        background: `radial-gradient(circle, rgba(220,190,255,${0.04 + Math.sin(phase * 0.3) * 0.015}) 0%, rgba(180,160,255,${0.02 + Math.sin(phase * 0.2) * 0.008}) 40%, transparent 70%)`,
        filter: "blur(80px)",
      ...
      <div style={{
        ...
        borderRadius: "50%",
        background: `radial-gradient(circle, rgba(200,240,255,${0.025 + Math.sin(phase * 0.4) * 0.01}) 0%, transparent 70%)`,
        filter: "blur(40px)",
        transform: `translate(${Math.sin(phase * 0.15) * 30}px, ${Math.cos(phase * 0.12) * 20}px)`,
      }} />
...
// ─────────────────────────────────────────────
const DEMO_NAMES = ["Alex", "Jordan", "Morgan", "Casey", "Riley", "Quinn", "Avery", "Blake", "Drew", "Skyler"];
const DEMO_COMPANIES = ["Rocketship", "Synapse AI", "Stackflow", "Orbit Labs", "Pulsar Inc.", "Nimbus Co.", "Vortex HQ", "Zenith Corp.", "Epoch Tech", "Solaris SaaS"];
...
// ─────────────────────────────────────────────
// DASH CARD WITH GRADIENT GLOW HOVER
// ─────────────────────────────────────────────
function DashCardWithGlow({ gc, vis, delay, children }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: vis ? 1 : 0,
        transform: vis ? (hovered ? "translateY(-3px) scale(1.02)" : "translateY(0)") : "translateY(24px)",
        transition: `opacity .6s ${delay}ms, transform .6s ${delay}ms, background .25s, border-color .25s, box-shadow .25s`,
        boxShadow: hovered
          ? `0 0 0 1px rgba(255,255,255,0.12), 0 6px 24px rgba(255,255,255,0.06), inset 0 1px 0 rgba(255,255,255,0.08)`
          : "none",
        cursor: "default",
      }}
    >
  ...
  return (
    <div ref={ref} style={{ maxWidth: 1100, margin: "0 auto", padding: "0 48px" }}>
      ...
      <div className="dashboard-cards-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        {cards.map((card, i) => {
          const glowColors = [
            { glow: "rgba(99,102,241,0.45)", border: "rgba(99,102,241,0.5)", bg: "rgba(99,102,241,0.06)", grad: "linear-gradient(135deg,rgba(99,102,241,0.15) 0%,rgba(168,85,247,0.08) 100%)" },
            { glow: "rgba(34,197,94,0.4)", border: "rgba(34,197,94,0.5)", bg: "rgba(34,197,94,0.05)", grad: "linear-gradient(135deg,rgba(34,197,94,0.12) 0%,rgba(16,185,129,0.07) 100%)" },
            { glow: "rgba(249,115,22,0.4)", border: "rgba(249,115,22,0.5)", bg: "rgba(249,115,22,0.05)", grad: "linear-gradient(135deg,rgba(249,115,22,0.12) 0%,rgba(234,179,8,0.07) 100%)" },
            { glow: "rgba(168,85,247,0.4)", border: "rgba(168,85,247,0.5)", bg: "rgba(168,85,247,0.05)", grad: "linear-gradient(135deg,rgba(168,85,247,0.12) 0%,rgba(236,72,153,0.07) 100%)" },
          ];
          const gc = glowColors[i];
          return (
          <DashCardWithGlow key={i} gc={gc} vis={vis} delay={i * 80}>
...
function WorkflowStep({ step, idx, total }) {
  ...
  return (
    <div ref={ref} className="workflow-step-grid" style={{
      display: "grid",
      ...
      <div className="workflow-step-text" style={{ order: isEven ? 1 : 2 }}>
      ...
      {/* Visual */}
      <div className="workflow-step-visual" style={{ order: isEven ? 2 : 1, height: 320, borderRadius: 18, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.015)", boxShadow: "0 24px 80px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.06)", padding: 4 }}>
        {step.visual()}
      </div>
    </div>
  );
...
function StepSlideshow() {
  ...
  const SLIDES = [
    ...
    },
    {
      ...
      body: lang === "sv" ? "Lägg till företagsnamn och kontakter. Logoplacers hämtar automatiskt varje logotyp direkt — vilket bolag som helst." : "Add company names and contacts. Logoplacers auto-fetches every logo instantly — any company, anywhere.",
      icon: Icon.users,
      visual: "list",
    },
    {
  ...
  const renderVisual = (type) => {
    ...
    if (type === "list") return (
      <div style={{ ...glassStyle, padding: "20px", height: "100%", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        {[
          { co: "Rocketship", name: "Marcus L.", status: true },
          { co: "Synapse AI", name: "Emma K.", status: true },
          { co: "Orbit Labs", name: "Johan A.", status: true },
          { co: "Stackflow", name: "Sarah M.", status: false },
          { co: "Nimbus Co.", name: "Erik B.", status: true },
        ].map((p, i) => (
      ...
      <div style={{ ...glassStyle, padding: "20px", height: "100%", position: "relative" }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, height: "65%", marginBottom: 12, overflow: "hidden", position: "relative" }}>
          ...
          <div style={{ position: "absolute", bottom: 16, right: 16, border: "1.5px dashed rgba(255,255,255,0.3)", borderRadius: 8, padding: "8px 12px", background: "rgba(255,255,255,0.04)" }}>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", textAlign: "center" }}>Rocketship<br />logo here</div>
          </div>
        </div>
        {/* Text layer indicator */}
        <div style={{ ...glassStyle, padding: "8px 12px", fontSize: 11, color: "rgba(255,255,255,0.55)" }}>
          Hi <span style={{ color: "#fff", fontWeight: 700 }}>Marcus</span>, see how <span style={{ color: "#fff", fontWeight: 700 }}>Rocketship</span> could…
        </div>
      </div>
    );
    if (type === "preview") return (
      <div style={{ ...glassStyle, padding: "16px", height: "100%", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, flex: 1 }}>
          {["Rocketship", "Synapse AI", "Orbit Labs", "Stackflow"].map((co, i) => (
    ...
    if (type === "send") return (
      <div style={{ ...glassStyle, padding: "20px", height: "100%", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {/* Send progress */}
        {["Rocketship — Jared C.", "Synapse AI — Emma K.", "Orbit Labs — Johan A."].map((r, i) => (
  ...
  return (
    <div ref={ref} style={{
      ...
      {/* Main slide layout — text left, glass visual right */}
...
// ─────────────────────────────────────────────
// FEATURES BENTO — interactive animated grid
// ─────────────────────────────────────────────
function FeatureBentoCard({ icon, title, desc, idx, visible, accentColor, wide = false, tall = false, visual = null }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${mouse.inside ? "rgba(255,255,255,0.18)" : "rgba(255,255,255,0.07)"}`,
        borderRadius: 20,
        padding: wide ? "28px 32px" : "28px 26px",
        gridColumn: wide ? "span 2" : "span 1",
        gridRow: tall ? "span 2" : "span 1",
        transition: `opacity .7s ${delay}ms, transform .7s ${delay}ms, background .25s, border-color .25s, box-shadow .25s`,
        opacity: visible ? 1 : 0,
        transform: visible ? (mouse.inside ? "translateY(-3px)" : "translateY(0)") : "translateY(28px)",
        boxShadow: mouse.inside
          ? `0 0 0 1px rgba(255,255,255,0.12), 0 8px 24px rgba(255,255,255,0.06), inset 0 1px 0 rgba(255,255,255,0.06)`
          : "none",
        cursor: "default",
    ...
    >
      ...
      <div style={{
        ...
        background: mouse.inside ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.05)",
        border: `1px solid ${mouse.inside ? "rgba(255,255,255,0.25)" : "rgba(255,255,255,0.08)"}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        color: mouse.inside ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.4)",
        transition: "all .25s",
        position: "relative", zIndex: 1,
        boxShadow: mouse.inside ? "0 0 14px rgba(255,255,255,0.08)" : "none",
      }}>{icon}</div>

      <div style={{ position: "relative", zIndex: 1, flex: 1 }}>
        <div style={{
          fontSize: 14, fontWeight: 700, color: mouse.inside ? "#fff" : "rgba(255,255,255,0.85)",
          marginBottom: 8, letterSpacing: "-.3px", transition: "color .2s",
        }}>{title}</div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.38)", lineHeight: 1.72 }}>{desc}</div>
      </div>

      {/* Optional visual element for wide cards */}
      {visual && (
...
function FeaturesShowcase() {
  ...
  useEffect(() => {
    const t = setInterval(() => setDemoCount(n => n + Math.floor(Math.random() * 3 + 1)), 4200);
    return () => clearInterval(t);
  }, []);
  ...
  const features = [
    {
      icon: Icon.bolt,
      title: lang === "sv" ? "Personalisering med ett klick" : "One-click personalisation",
      desc: lang === "sv" ? "Ladda upp en gång. Logoplacers hämtar automatiskt din prospekts logotyp och placerar den perfekt på din demo — varje gång." : "Upload once. Logoplacers auto-fetches your prospect's logo and places it perfectly on your demo — every single time.",
      accent: "#818cf8", wide: true,
      visual: (
        <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8, flexWrap: "wrap" }}>
          {["Rocketship", "Synapse AI", "Orbit Labs", "Nimbus Co."].map((co, i) => (
            <div key={i} style={{
              ...
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: ["#818cf8","#34d399","#fb7185","#fbbf24"][i], boxShadow: `0 0 6px ${["#818cf8","#34d399","#fb7185","#fbbf24"][i]}` }} />
              {co}
            </div>
          ))}
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.2)", marginLeft: 2 }}>+∞ more</div>
        </div>
      ),
    },
    {
      ...
      desc: lang === "sv" ? "Ansluten till ditt Gmail. 100 personaliserade mejl med naturliga fördröjningar — utan att lämna verktyget." : "Connected to your Gmail. 100 personalised emails with natural delays — without ever leaving the tool.",
      accent: "#34d399",
      visual: (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
          <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#34d399", boxShadow: "0 0 8px #34d399", animation: "glowPulse 2s ease-in-out infinite" }} />
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{lang === "sv" ? "Skickar med mänskliga fördröjningar" : "Sending with human-like delays"}</span>
        </div>
      ),
    },
    {
      icon: Icon.target,
      title: lang === "sv" ? "Pixelperfekt placering" : "Pixel-perfect placement",
      desc: lang === "sv" ? "Dra, ändra storlek och placera varje element med precision. Din demo ser exakt ut som du tänkt för varje mottagare." : "Drag, resize and position every element with precision. Your demo looks exactly how you intended for every single recipient.",
      accent: "#fb7185",
    },
    {
      icon: Icon.search,
      title: lang === "sv" ? "Smart logotypdetektering" : "Smart logo detection",
      desc: lang === "sv" ? "Skriv ett företagsnamn och Logoplacers hittar och hämtar rätt varumärkeslogotyp automatiskt. Ingen manuell sökning." : "Type a company name and Logoplacers automatically finds and fetches the correct brand logo. No manual searching.",
      accent: "#fbbf24", wide: true,
      visual: (
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4 }}>
          <div style={{ flex: 1, height: 6, background: "rgba(255,255,255,0.06)", borderRadius: 3, overflow: "hidden" }}>
            <div style={{ width: "85%", height: "100%", background: "linear-gradient(90deg, #fbbf24, #f97316)", borderRadius: 3, animation: "featurePulse 2s ease-in-out infinite" }} />
          </div>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", whiteSpace: "nowrap" }}>1.2s avg</span>
        </div>
      ),
    },
    {
      icon: Icon.box,
      title: lang === "sv" ? "Bulkexport på sekunder" : "Bulk export in seconds",
      desc: lang === "sv" ? "Generera personaliserade bilder för 100 prospekter på den tid det brukade ta att göra en. ZIP eller direktutskick." : "Generate personalised images for 100 prospects in the time it used to take to do one. ZIP download or direct send.",
      accent: "#a78bfa",
      visual: (
        <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 }}>
          <span style={{ fontSize: 22, fontWeight: 800, color: "#a78bfa", letterSpacing: "-1px", lineHeight: 1 }}>{demoCount.toLocaleString()}</span>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>{lang === "sv" ? "demos skapade" : "demos created"}</span>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#a78bfa", boxShadow: "0 0 6px #a78bfa", animation: "glowPulse 1.5s ease-in-out infinite", marginLeft: 2 }} />
        </div>
      ),
    },
    {
      icon: Icon.lock,
      title: lang === "sv" ? "Säkert & privat" : "Secure & private",
      desc: lang === "sv" ? "Dina Gmail-uppgifter och e-postinnehåll lämnar aldrig din webbläsare. Kontodata lagras säkert på server." : "Your Gmail credentials and email content never leave your browser. Account data stored securely on our servers.",
      accent: "#22d3ee",
      visual: (
        <div style={{ display: "flex", gap: 6, marginTop: 4, flexWrap: "wrap" }}>
          {(lang === "sv" ? ["Säker server", "Krypterat", "GDPR"] : ["Secure server", "Encrypted", "GDPR"]).map((tag, i) => (
            <div key={i} style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.5px", padding: "3px 8px", borderRadius: 100, background: "rgba(34,211,238,0.08)", border: "1px solid rgba(34,211,238,0.2)", color: "rgba(34,211,238,0.7)" }}>{tag}</div>
          ))}
        </div>
      ),
    },
  ...
  const pills = [
    { label: lang === "sv" ? "Automatisk logotyp" : "Auto logo fetch", color: "#818cf8" },
    { label: lang === "sv" ? "Bulk 100 demos" : "Bulk 100 demos", color: "#34d399" },
    { label: lang === "sv" ? "Gmail integrerat" : "Gmail integrated", color: "#fb7185" },
    { label: lang === "sv" ? "Under 10 min" : "Under 10 min", color: "#fbbf24" },
  ];
...
// ─────────────────────────────────────────────
const TDATA = [
  { quote: "Getting a demo built for your company — that's insanely cool. I'll use this every single day.", quoteSv: "Att få en demo byggd för sitt bolag — det är sjukt snyggt. Jag kommer använda detta varje dag.", name: "Hannes H.", role: "User", roleSv: "Användare", company: "Logoplacers" },
  { quote: "As a founder this is exactly what I needed. Personalised demos at scale without any design work.", quoteSv: "Som grundare är det precis det här jag behövde. Personaliserade demos i skala utan designarbete.", name: "Kasper Frelén", role: "Owner", roleSv: "Grundare", company: "Foundry" },
...
// SOCIAL PROOF TICKER
// ─────────────────────────────────────────────
function AnimatedStat({ target, suffix, prefix, lbl, visible, delay = 0, mouseX, mouseY, idx, grad, staticDisplay }) {
  ...
  return (
    <div style={{ padding: "32px 24px", borderRadius: 16, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", transition: "transform .2s, box-shadow .2s" }}
      onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 8px 32px rgba(255,255,255,0.06)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.15)"; e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
      onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.background = "rgba(255,255,255,0.04)"; }}>
      <div style={{ fontSize: "clamp(40px,6vw,64px)", fontWeight: 800, letterSpacing: "-2px", background: grad || "linear-gradient(135deg,#fff,rgba(255,255,255,0.5))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", lineHeight: 1 }}>
        {staticDisplay ? `${prefix}${target}${suffix}` : `${prefix}${count}${suffix}`}
      </div>
...
// ─────────────────────────────────────────────
const TICKS = ["Marcus sent 47 demos in 6 minutes", "Rocketship logo fetched in 1.2s", "Emma's reply rate: 28%", "100 personalised emails sent", "Sofia saved 8 hours this week"];
const TICKS_SV = ["Marcus skickade 47 demos på 6 min", "Rocketship logotyp hämtades på 1.2s", "Emmas svarsfrekvens: 28%", "100 personaliserade mejl skickade", "Sofia sparade 8 timmar den här veckan"];
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      <div className="live-demo-grid" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 28, alignItems: "start" }}>
        ...
        <div className="live-demo-canvas-wrap" style={{
          ...
          boxShadow: "0 40px 80px rgba(0,0,0,0.55), 0 0 0 1px rgba(220,190,255,0.15), 0 0 40px rgba(220,190,255,0.08), 0 0 80px rgba(180,160,255,0.06)",
        ...
        <div style={{
          ...
          {[
            { label: lang === "sv" ? "Företag" : "Company", placeholder: "e.g. Rocketship", val: company, set: setCompany, type: "text" },
...
const DEMO_STEPS = [
  ...
  },
  {
    ...
    color: "rgba(255,255,255,0.7)",
    leads: [
      { company: "Rocketship", name: "Jared Cole", email: "jared@rocketship.io", status: "ok" },
      { company: "Synapse AI", name: "Gavin Price", email: "gavin@synapse.ai", status: "ok" },
      { company: "Stackflow", name: "Erik M.", email: "erik@stackflow.co", status: "ok" },
      { company: "Orbit Labs", name: "Michael B.", email: "michael@orbitlabs.io", status: "ok" },
      { company: "Nimbus Co.", name: "Richard K.", email: "richard@nimbus.co", status: "ok" },
    ],
  },
  {
...
function DemoCanvas({ step, activeLeadIdx }) {
  ...
  useEffect(() => {
    ...
    if (step === 2) {
      ...
      ctx.fillText("Demo — Rocketship", 275, 120);
      ctx.fillStyle = "#64748b"; ctx.font = "10px 'DM Sans',sans-serif";
      ctx.fillText("Hi Marcus, here's what Rocketship could look like in your workflow", 275, 137);
      ...
      ["TEXT LAYERS", "RECIPIENT LOGO", "STORLEK"].forEach((label, i) => {
        ...
        if (i === 1) { ctx.fillStyle = "#1d4ed8"; ctx.font = "600 11px sans-serif"; ctx.fillText("Rocketship logo ✓", 278, ly + 28); }
        if (i === 2) {
      ...
      ctx.textAlign = "center"; ctx.fillText("Rocketship", 472, 264); ctx.textAlign = "left";
      // Drag handles
      ...
      ctx.fillText("Hi Marcus — see Rocketship's data", 436, 310);
      ctx.fillStyle = "#64748b"; ctx.font = "10px 'DM Sans',sans-serif";
    ...
    if (step === 3) {
      ...
      const demoCompanies = ["Rocketship", "Synapse AI", "Stackflow", "Orbit Labs", "Pulsar Inc.", "Nimbus Co.", "Vortex HQ", "Zenith Corp."];
      ...
      demoCompanies.slice(0, cols * rows).forEach((co, i) => {
        ...
        // Fake logo blob
        const colors = { "Rocketship": "rgba(240,200,255,0.85)", "Synapse AI": "#818cf8", "Stackflow": "#f59e0b", "Orbit Labs": "#10b981", "Pulsar Inc.": "#8b5cf6", "Nimbus Co.": "#e0115f", "Vortex HQ": "#0099ff", "Zenith Corp.": "#f97316" };
    ...
    if (step === 4) {
      ...
      ctx.fillStyle = "#fff"; ctx.font = "600 11px 'DM Sans',sans-serif"; ctx.fillText("Marcus L.", vx + 36, vy + 16);
      ...
      ctx.textAlign = "center"; ctx.fillText("Marcus' pipeline", fx + 10 + chipW / 2, fy + fh - 12); ctx.textAlign = "left";
      // Play button
      ...
      // Website content (fake landing page of Rocketship)
      ...
      ctx.textAlign = "center"; ctx.fillText("Rocketship", wx2 + ww / 2, wy2 + 17); ctx.textAlign = "left";
...
// ─────────────────────────────────────────────
const HI_PERSONAS = [
  { name: "Emma", nameSv: "Emma", company: "Rocketship", initial: "R", color: "#3b82f6" },
  { name: "Marcus", nameSv: "Marcus", company: "Synapse AI", initial: "S", color: "#22c55e" },
  { name: "Sofia", nameSv: "Sofia", company: "Stackflow", initial: "S", color: "#f97316" },
  { name: "Johan", nameSv: "Johan", company: "Orbit Labs", initial: "O", color: "#a855f7" },
  { name: "Anna", nameSv: "Anna", company: "Pulsar Inc.", initial: "P", color: "#ef4444" },
  { name: "Erik", nameSv: "Erik", company: "Nimbus Co.", initial: "N", color: "#06b6d4" },
  { name: "Lena", nameSv: "Lena", company: "Vortex HQ", initial: "V", color: "#f59e0b" },
  { name: "Oscar", nameSv: "Oscar", company: "Zenith Corp.", initial: "Z", color: "#10b981" },
];
...
export default function Landing({ onEnterApp, onOpenBlog }) {
  ...
  // ── SEO + AIO structured data & meta ───────────────────────────
  useEffect(() => {
    const isEn = lang !== "sv";
    const title = isEn
      ? "Logoplacers — Auto Logo Fetch & Personalised Sales Demos via Gmail"
      : "Logoplacers — Automatisk logotyphämtning & personaliserade säljdemos via Gmail";
    const desc = isEn
      ? "Paste your prospect list, auto-fetch every company logo, and send 100 personalised demo images directly from Gmail in under 10 minutes. No design skills needed. Free to start."
      : "Klistra in din prospektlista, hämta varje logotyp automatiskt och skicka 100 personaliserade demos från Gmail på under 10 minuter. Inga designkunskaper krävs. Gratis att börja.";
    const keywords = "personalised sales emails, logo personalisation, B2B outbound, sales demo automation, Gmail outreach tool, cold email personalisation, prospect logo fetch, automated sales demos";
    ...
    setMeta("author", "Logoplacers");
    // Open Graph
    setMeta("og:title", title, true);
    setMeta("og:description", desc, true);
    setMeta("og:type", "website", true);
    setMeta("og:site_name", "Logoplacers", true);
    setMeta("og:url", `https://logoplacers.com${isEn ? "" : "/sv"}`, true);
    // Twitter card
    setMeta("twitter:card", "summary_large_image");
    setMeta("twitter:title", title);
    setMeta("twitter:description", desc);
    setMeta("twitter:site", "@logoplacers");
    // Canonical
    let canon = document.querySelector("link[rel='canonical']");
    if (!canon) { canon = document.createElement("link"); canon.rel = "canonical"; document.head.appendChild(canon); }
    canon.href = `https://logoplacers.com${isEn ? "" : "/sv"}`;
    ...
    const schemas = [
      {
        ...
        data: {
          ...
          "description": desc,
          "offers": [
            { "@type": "Offer", "price": "0", "priceCurrency": "USD", "name": "Free — 4 credits/day" },
            { "@type": "Offer", "price": "19", "priceCurrency": "USD", "name": "SDR — 300 credits/month", "billingIncrement": "month" },
            { "@type": "Offer", "price": "29", "priceCurrency": "USD", "name": "Sales Pro — 2000 credits/month", "billingIncrement": "month" },
            { "@type": "Offer", "price": "59", "priceCurrency": "USD", "name": "Team — 10000 credits/month", "billingIncrement": "month" }
          ],
          "featureList": [
            "Automatic company logo detection from name or domain",
            "Bulk personalised image generation (100+ per batch)",
            "Send directly from Gmail with anti-spam delays",
            "Video personalisation with recipient logo overlay",
            "No design skills required",
            "Supports PNG, JPG, WEBP, HEIC formats",
            "GDPR compliant — Gmail credentials never stored server-side"
          ]
        }
      },
      {
        ...
        data: {
          ...
          "@type": "FAQPage",
          "mainEntity": [
            { "@type": "Question", "name": isEn ? "Does Logoplacers store my Gmail credentials?" : "Lagrar Logoplacers mina Gmail-uppgifter?", "acceptedAnswer": { "@type": "Answer", "text": isEn ? "No. Your Gmail connection uses Google's official OAuth flow entirely in the browser. We never see or store your credentials or email content." : "Nej. Din Gmail-anslutning använder Googles officiella OAuth-flöde helt i webbläsaren. Vi ser eller lagrar aldrig dina uppgifter eller e-postinnehåll." } },
            { "@type": "Question", "name": isEn ? "How many prospects can I personalise at once?" : "Hur många prospekter kan jag personalisera på en gång?", "acceptedAnswer": { "@type": "Answer", "text": isEn ? "There is no hard limit. Logoplacers generates images for every prospect in your list and exports them as a ZIP, or sends them directly via Gmail with anti-spam delays." : "Det finns ingen hård gräns. Logoplacers genererar bilder för varje prospekt i din lista och exporterar dem som en ZIP, eller skickar dem direkt via Gmail med anti-spam-fördröjningar." } },
            { "@type": "Question", "name": isEn ? "How does automatic logo detection work?" : "Hur fungerar automatisk logotypdetektering?", "acceptedAnswer": { "@type": "Answer", "text": isEn ? "Type a company name or domain and Logoplacers queries multiple logo databases simultaneously. It validates each result and falls back gracefully if a logo cannot be found." : "Skriv ett företagsnamn eller domän och Logoplacers söker i flera logotypdatabaser samtidigt. Det validerar varje resultat och faller tillbaka elegant om ingen logotyp hittas." } },
            { "@type": "Question", "name": isEn ? "Is there a free trial?" : "Finns det en gratis provperiod?", "acceptedAnswer": { "@type": "Answer", "text": isEn ? "Yes — the Free plan gives you 4 credits per day with no credit card required. Paid plans start at $19/month for 300 credits." : "Ja — gratisplanen ger dig 4 krediter per dag utan kreditkort. Betalplaner börjar på $19/månad för 300 krediter." } },
            { "@type": "Question", "name": isEn ? "What image formats does it support?" : "Vilka bildformat stöds?", "acceptedAnswer": { "@type": "Answer", "text": isEn ? "PNG, JPG, WEBP and HEIC (iPhone photos). HEIC files are automatically converted in the browser — no external tool needed." : "PNG, JPG, WEBP och HEIC (iPhone-foton). HEIC-filer konverteras automatiskt i webbläsaren — inget externt verktyg behövs." } }
          ]
        }
      },
      {
        ...
        data: {
          ...
          "name": "Logoplacers",
          "url": "https://logoplacers.com",
          "contactPoint": { "@type": "ContactPoint", "email": "adminlogoplacers@gmail.com", "contactType": "customer support" }
        }
      }
    ];
      ...
      <div data-theme={darkMode ? "dark" : "light"} className="scroll-glow-bg" style={{ background: darkMode ? "#000" : "#fafaf8", color: darkMode ? "#fff" : "#0a0a0a", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", overflowX: "hidden", transition: "background .4s, color .4s" }}>
          ...
          <div className="nav-links" style={{ display: "flex", alignItems: "center", gap: 4 }}>
            ...
            <div style={{ display: "flex", gap: 2, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: 2, marginLeft: 4 }}>
              {[["en", "English"], ["sv", "Svenska"]].map(([l, label]) => (
                <button key={l} onClick={() => setLang(l)} style={{ background: lang === l ? "rgba(255,255,255,0.12)" : "none", border: "none", borderRadius: 6, padding: "3px 9px", color: lang === l ? "#fff" : "rgba(255,255,255,0.35)", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all .15s" }}>{label}</button>
              ))}
            </div>
          ...
          <div className="hero-grid" style={{
            ...
            <div style={{ animation: "fadeSlideUp .8s ease both" }}>
              ...
              <h1 style={{ fontSize: "clamp(36px, 4.5vw, 58px)", fontWeight: 800, lineHeight: 1.05, letterSpacing: "-2.5px", margin: "0 0 20px" }}>
                ...
                <span style={{ color: "#fff", display: "block" }}>
                  {lang === "sv" ? "Varje mejl." : "Every email."}
                </span>
              </h1>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
                <button onClick={onEnterApp} style={{
                  background: "#fff", color: "#000", border: "none", borderRadius: 12, padding: "15px 36px",
                ...
                <button onClick={() => document.getElementById("how-it-works")?.scrollIntoView({ behavior: "smooth" })} style={{
                  ...
                  border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "15px 24px",
                  ...
                  <span style={{ width: 20, height: 20, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.25)", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <svg width="7" height="7" viewBox="0 0 10 10" fill="rgba(255,255,255,0.7)" xmlns="http://www.w3.org/2000/svg"><path d="M2 1.5 L8.5 5 L2 8.5 Z" /></svg>
                  </span>
              ...
              {/* Social proof strip */}
              <div style={{ display: "flex", alignItems: "center", gap: 14, fontSize: 12, color: "rgba(255,255,255,0.28)" }}>
                <div style={{ display: "flex" }}>
                  {["W","P","G","I","A"].map((initial, i) => (
                    <div key={i} style={{ width: 26, height: 26, borderRadius: "50%", background: ["#3b82f6","#22c55e","#f97316","#a855f7","#06b6d4"][i], border: "2px solid #000", marginLeft: i > 0 ? -8 : 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "#fff" }}>{initial}</div>
                  ))}
                </div>
                {lang === "sv" ? "Används av säljteam varje dag" : "Used by sales teams every day"}
              </div>
            </div>
        ...
        <section aria-label="Key metrics" style={{ padding: "80px 48px", background: "#0a0a0a", borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ maxWidth: 1040, margin: "0 auto" }}>
            <div className="metrics-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 2 }}>
              {[
                {
                  value: "< 1s", label: lang === "sv" ? "per personalisering" : "per personalisation", desc: lang === "sv" ? "Automatisk logotyphämtning" : "Automatic logo fetch",
                ...
                {
                  value: "94%", label: lang === "sv" ? "föredrar visuella demos" : "prefer visual demos", desc: lang === "sv" ? "Av B2B-köpare" : "Of B2B buyers",
                ...
                {
                  value: "1 000+", label: lang === "sv" ? "demos per batch" : "demos per batch", desc: lang === "sv" ? "Ingen hård gräns" : "No hard limit",
                  icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>
                },
                {
                  value: "< 10 min", label: lang === "sv" ? "till skickade mejl" : "to sent emails", desc: lang === "sv" ? "Från skärmbild" : "From screenshot",
                ...
                <div key={i} style={{
                  ...
                  <div style={{ marginBottom: 14, opacity: 0.45, color: "rgba(255,255,255,0.9)", display: "flex", alignItems: "center" }}>{m.icon}</div>
        ...
        <section id="cta" aria-label="Call to action" ref={ctaRef} style={{ padding: "140px 48px 160px", borderTop: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: 560, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 2, opacity: ctaVis ? 1 : 0, transform: ctaVis ? "translateY(0)" : "translateY(28px)", transition: "opacity .9s, transform .9s" }}>
            ...
            <p style={{ fontSize: 15, color: "rgba(255,255,255,0.38)", marginBottom: 40, lineHeight: 1.7 }}>
              {lang === "sv"
                ? "Kom igång gratis på under 2 minuter. Inget kreditkort. Inga designkunskaper. Bara fler svar."
                : "Get started free in under 2 minutes. No credit card. No design skills. Just more replies."}
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
              <button onClick={onEnterApp}
                style={{
                  background: "linear-gradient(105deg, #fff 0%, #e8c8ff 30%, #f8c8ff 60%, #fff 100%)",
                  backgroundSize: "200% auto", animation: "cdShimmer 3s linear infinite",
                  color: "#000", border: "none", borderRadius: 14, padding: "18px 52px",
                  fontSize: 16, fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
                  letterSpacing: "-.2px", transition: "box-shadow .2s, transform .15s",
                  boxShadow: "0 0 40px rgba(220,190,255,0.25)",
                  width: "100%", maxWidth: 360,
                }}
                onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 8px 48px rgba(220,190,255,0.5)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow = "0 0 40px rgba(220,190,255,0.25)"; e.currentTarget.style.transform = "none"; }}>
                {lang === "sv" ? "Börja gratis — inget kreditkort" : "Start free — no credit card"}
              </button>
              <div style={{ display: "flex", alignItems: "center", gap: 20, marginTop: 8 }}>
                {[
                  lang === "sv" ? "4 krediter/dag gratis" : "4 credits/day free",
                  lang === "sv" ? "Anslut Gmail med ett klick" : "Connect Gmail in one click",
                  lang === "sv" ? "Avbryt när som helst" : "Cancel anytime",
                ].map((item, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "rgba(255,255,255,0.3)" }}>
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
                    {item}
                  </div>
                ))}
              </div>
        ...
        <section id="pricing" style={{ padding: "100px 48px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.012)" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            ...
            <div className="pricing-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(185px,1fr))", gap: 12 }}>
              {[
        ...
        @media (max-width: 768px) {
          ...
          .hero-demo-card > div { min-height: 300px !important; }

          /* Key metrics: 2×2 grid */
          .metrics-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .metrics-grid > div { border-left: none !important; border-top: 1px solid rgba(255,255,255,0.06) !important; padding: 24px 18px !important; }
          .metrics-grid > div:nth-child(1),
          .metrics-grid > div:nth-child(2) { border-top: none !important; }
          .metrics-grid > div:nth-child(odd) { border-right: 1px solid rgba(255,255,255,0.06) !important; }

          /* Workflow steps: single column, text always on top */
          .workflow-step-grid { grid-template-columns: 1fr !important; gap: 24px !important; padding: 48px 0 !important; }
          .workflow-step-text { order: 1 !important; }
          .workflow-step-visual { order: 2 !important; height: 260px !important; }

          /* Stats */
          .dashboard-cards-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .stats-numbers-grid { grid-template-columns: repeat(2, 1fr) !important; }
          ...
          /* Features bento rows → single column */
          .features-row-2col { grid-template-columns: 1fr !important; }
          .features-row-2col > div { border-radius: 16px !important; }
          .features-row-3col { grid-template-columns: 1fr !important; }
          .features-row-3col > div { border-radius: 16px !important; }
          ...
          .pricing-grid > div[style*="scale(1.03)"] { transform: scale(1) !important; }

          /* Testimonial cards: narrower */
          .testi-stage { height: 360px !important; }
          .testi-card { width: 300px !important; padding: 28px 24px 24px !important; }
          ...
          /* Footer: stack */
          footer { flex-direction: column !important; align-items: flex-start !important; padding: 28px 20px !important; gap: 20px !important; }
          footer > div:last-child { flex-wrap: wrap !important; gap: 12px !important; }
        ...
        @media (max-width: 480px) {
          ...
          .workflow-step-visual { height: 220px !important; }

          /* Metrics: full single column */
          .metrics-grid { grid-template-columns: 1fr !important; }
          .metrics-grid > div { border-right: none !important; border-top: 1px solid rgba(255,255,255,0.06) !important; }
          .metrics-grid > div:first-child { border-top: none !important; }

          /* Testimonial cards */
          .testi-stage { height: 320px !important; }
          .testi-card { width: 270px !important; padding: 22px 18px 20px !important; }
        }
