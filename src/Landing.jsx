        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", lineHeight: 1.6 }}>
              {lang === "sv" ? <>I det riktiga verktyget, klistra in <strong style={{ color: "#5ba4ff" }}>100 företag på en gång</strong> och få 100 personaliserade demos på sekunder.</> : <>In the real tool, paste <strong style={{ color: "#5ba4ff" }}>100 companies at once</strong> and get 100 personalised demos in seconds.</>}
            </div>
          </div>
...
const TDATA = [
  ...
  { quote: "The Gmail integration is a game changer. Prospects respond saying they were impressed by how tailored the outreach felt.", quoteSv: "Gmail-integrationen är revolutionerande. Prospects svarar och säger att de var imponerade av hur skräddarsydd outreachen kändes.", name: "Sofia R.", role: "SDR Lead", roleSv: "SDR Lead", company: "B2B Fintech", c: "#8b5cf6" },
  { quote: "Every cold email tool promises personalisation. This is the first one that makes it visual. Like Loom but for demos.", quoteSv: "Varje cold email-verktyg lovar personalisering. Det här är det första som gör det visuellt. Som Loom men för demos.", name: "Erik J.", role: "Head of Sales", roleSv: "Säljchef", company: "Growth Agency", c: "#10b981" },
  { quote: "We went from 4% reply rate to 18% in two weeks. Visual personalisation makes prospects feel you really did your homework.", quoteSv: "Vi gick från 4% svarsfrekvens till 18% på två veckor. Visuell personalisering får prospects att känna att du verkligen gjort din research.", name: "Anna K.", role: "VP Sales", roleSv: "VP Försäljning", company: "SaaS Startup", c: "#f59e0b" },
  { quote: "Logo auto-detection saves me 20 minutes per prospect. I now send personalised demos to everyone on my list.", quoteSv: "Automatisk logotypdetektering sparar mig 20 minuter per prospekt. Nu skickar jag personaliserade demos till alla på min lista.", name: "Johan B.", role: "Account Executive", roleSv: "Account Executive", company: "B2B Tech", c: "#ec4899" },
  { quote: "Outreach that actually stands out. Our SDR team adopted it in one day — zero training needed.", quoteSv: "Outreach som faktiskt sticker ut. Vårt SDR-team tog det i bruk på en dag — noll träning behövdes.", name: "Lena M.", role: "Sales Manager", roleSv: "Säljansvarig", company: "Enterprise SaaS", c: "#06b6d4" },
];
const STAR_ROW = (
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: 60 }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "2px", color: "#1a82ff", textTransform: "uppercase", marginBottom: 14 }}>{lang === "sv" ? "Vad folk säger" : "What people say"}</div>
        <h2 style={{ fontSize: "clamp(32px,5vw,52px)", fontWeight: 800, letterSpacing: "-2px", margin: 0 }}>{lang === "sv" ? "Säljteam älskar det." : "Sales teams love it."}</h2>
      </div>
      {/* Main card */}
      <div style={{ position: "relative", marginBottom: 24 }}>
        <div style={{
          background: "rgba(255,255,255,0.022)", backdropFilter: "blur(32px)", WebkitBackdropFilter: "blur(32px)",
          border: `1px solid ${cur.c}44`, borderRadius: 28,
          padding: "clamp(28px,4vw,52px) clamp(24px,5vw,60px)",
          boxShadow: `0 0 80px ${cur.c}14,0 24px 64px rgba(0,0,0,.32)`,
          ...
          <div style={{ position: "absolute", top: 10, left: 32, fontSize: 100, lineHeight: 1, color: `${cur.c}12`, fontFamily: "Georgia,serif", fontWeight: 700, pointerEvents: "none", userSelect: "none" }}>"</div>
          ...
          <p style={{ fontSize: "clamp(15px,2vw,20px)", color: "rgba(255,255,255,.83)", lineHeight: 1.65, marginBottom: 28, fontStyle: "italic", maxWidth: 800, position: "relative", margin: "0 0 28px" }}>
            "{lang === "sv" && cur.quoteSv ? cur.quoteSv : cur.quote}"
          </p>
          <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
            <div style={{ width: 42, height: 42, borderRadius: "50%", background: `linear-gradient(135deg,${cur.c},${cur.c}99)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800, color: "#fff", boxShadow: `0 0 16px ${cur.c}55`, flexShrink: 0 }}>{cur.name[0]}</div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{cur.name}</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,.32)", marginTop: 2 }}>{lang === "sv" && cur.roleSv ? cur.roleSv : cur.role} · {cur.company}</div>
            </div>
          </div>
        </div>
        {/* Arrows */}
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 7, marginBottom: 26 }}>
        {TDATA.map((_, i) => (
          <button key={i} onClick={() => go(i)} style={{ width: i === idx ? 22 : 7, height: 7, borderRadius: 4, background: i === idx ? cur.c : "rgba(255,255,255,.15)", border: "none", cursor: "pointer", padding: 0, transition: "width .3s,background .3s" }} />
        ))}
      </div>
      {/* Mini strip */}
...
const TICKS_SV = [
  "Marcus från Stockholm skickade precis 47 personaliserade demos",
  "Sofia stängde en deal efter sin logodemo · för 3 min sedan",
  "Eriks svarsfrekvens nådde 31% den här veckan",
  "Ny användare från London registrerade sig precis",
  "Anna skickade 120 demos på 8 minuter",
  "Johans prospect svarade inom 4 minuter",
  "Lenas team skickade 340 demos idag",
  "Ny användare från Berlin registrerade sig precis",
];
...
function ComparisonTable() {
  ...
  const rows = [
    ...
    { f: "Bulk personalisation", fSv: "Bulkpersonalisering", lp: 1, loom: 0, figma: 0, manual: 0 },
    { f: "Under 30 seconds per prospect", fSv: "Under 30 sekunder per prospekt", lp: 1, loom: 0, figma: 0, manual: 0 },
    { f: "No design skills needed", fSv: "Inga designkunskaper krävs", lp: 1, loom: 1, figma: 0, manual: 1 },
    { f: "Visual demo (not just text)", fSv: "Visuell demo (inte bara text)", lp: 1, loom: 1, figma: 1, manual: 0 },
    ...
    <div ref={ref} style={{ maxWidth: 840, margin: "0 auto", opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(28px)", transition: "opacity .7s,transform .7s" }}>
      ...
      <div style={{ overflowX: "auto", borderRadius: 20, border: "1px solid rgba(255,255,255,.07)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 520 }}>
          <thead>
            <tr>
              <th style={{ padding: "13px 18px", textAlign: "left", fontSize: 11, color: "rgba(255,255,255,.28)", fontWeight: 600, background: "rgba(255,255,255,.02)", borderBottom: "1px solid rgba(255,255,255,.06)" }}>{lang === "sv" ? "Funktion" : "Feature"}</th>
              {[{ l: "Logoplacers", hi: 1 }, { l: "Loom" }, { l: "Figma" }, { l: "Manual" }].map((c, i) => (
                <th key={i} style={{ padding: "13px 18px", textAlign: "center", fontSize: 12, fontWeight: 700, color: c.hi ? "#fff" : "rgba(255,255,255,.35)", background: c.hi ? "rgba(26,130,255,.1)" : "rgba(255,255,255,.02)", borderBottom: `2px solid ${c.hi ? "rgba(26,130,255,.5)" : "rgba(255,255,255,.05)"}` }}>
                  {c.l}{c.hi ? <span style={{ display: "block", fontSize: 9, color: "#5ba4ff", fontWeight: 600, marginTop: 2 }}>{lang === "sv" ? "← du är här" : "← you're here"}</span> : null}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((row, ri) => (
              <tr key={ri}>
                <td style={{ padding: "11px 18px", fontSize: 12, color: "rgba(255,255,255,.5)", borderBottom: "1px solid rgba(255,255,255,.04)", background: "rgba(255,255,255,.01)" }}>{lang === "sv" && row.fSv ? row.fSv : row.f}</td>
    ...
    <div style={{ background: "#070b12", color: "#fff", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", overflowX: "hidden" }}>
      ...
      <section style={{ padding: "80px 48px 60px", background: "rgba(255,255,255,.015)", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 920, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ marginTop: 32, display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            {(lang === "sv"
              ? ["Upp till 3x fler svar", "100 demos på 8 minuter", "Inga designkunskaper krävs", "Skickar från ditt eget Gmail"]
              : ["Reply rates up to 3x higher", "100 demos in 8 minutes", "Zero design skills needed", "Sends from your own Gmail"]
            ).map(b => (
      ...
      <section id="features" style={{ padding: "120px 48px" }}>
        <div style={{ maxWidth: 1140, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 80 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "2px", color: "#1a82ff", textTransform: "uppercase", marginBottom: 16 }}>{lang === "sv" ? "Funktioner" : "Features"}</div>
            <h2 style={{ fontSize: "clamp(32px,5vw,54px)", fontWeight: 800, letterSpacing: "-2px", margin: 0, lineHeight: 1.05 }}>
              {lang === "sv" ? "Allt du behöver för att" : "Everything you need"}<br />{lang === "sv" ? "sticka ut i inkorgen." : "to stand out in the inbox."}
            </h2>
          </div>
      ...
      <section id="waitlist" ref={ctaRef} style={{ padding: "140px 48px 160px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "2px", color: "#1a82ff", textTransform: "uppercase", marginBottom: 20 }}>{lang === "sv" ? "Tidig åtkomst" : "Early access"}</div>
          <h2 style={{ fontSize: "clamp(32px,5vw,54px)", fontWeight: 800, letterSpacing: "-2px", margin: "0 0 16px", lineHeight: 1.05 }}>
            {lang === "sv" ? "Var först i kön." : "Be first in line."}
          </h2>
          ...
          <div style={{ marginTop: 28, fontSize: 12, color: "rgba(255,255,255,0.2)" }}>
            {lang === "sv" ? "Inget spam. Inget kreditkort. Bara tidig åtkomst." : "No spam. No credit card. Just early access."}
          </div>
        </div>
      </section>
      ...
      <section id="faq" style={{ padding: "120px 48px", background: "rgba(255,255,255,0.018)" }}>
        <div style={{ maxWidth: 740, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 72 }}>
            ...
            <h2 style={{ fontSize: "clamp(32px,5vw,54px)", fontWeight: 800, letterSpacing: "-2px", margin: 0 }}>{lang === "sv" ? "Vanliga frågor." : "Common questions."}</h2>
          </div>
          <div ref={faqRef} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              ...
              { q: lang === "sv" ? "Finns det en gratis provperiod?" : "Is there a free trial?", a: lang === "sv" ? "Ja — gratisplanen ger dig 4 krediter per dag utan kreditkort. Betalplaner börjar på $19/månad för 300 krediter. Krediter förbrukas när du genererar personaliserade bilder eller skickar mejl." : "Yes — the Free plan gives you 4 credits per day with no credit card required. Paid plans start at $19/month for 300 credits. Credits are spent when you generate personalised images or send emails." },
            ].map((f, i) => <FAQ key={i} {...f} />)}
          </div>
        </div>
      </section>
